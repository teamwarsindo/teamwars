    ...
    <div className="bg-card border border-border p-3 sm:p-3.5 rounded-2xl shadow-xs space-y-2.5">
      ...
      <div className="grid grid-cols-3 gap-1.5 w-full">
        <button
          ...
          className={`py-2 px-1 rounded-xl text-[10.5px] font-bold transition cursor-pointer leading-snug truncate ${
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex-1" ref={teamRef}>
          <button
            ...
            className="w-full bg-background border border-input rounded-xl px-2.5 py-2 text-[11px] font-medium text-foreground flex items-center justify-between transition hover:border-primary cursor-pointer shadow-2xs"
          >
            <span className="truncate">
              {selectedTeamFilter === "ALL" ? "Semua Tim" : selectedTeamFilter}
            </span>
            <ChevronDown className={`h-3.5 w-3.5 text-muted-foreground transition-transform shrink-0 ${isTeamOpen ? "rotate-180" : ""}`} />
          </button>
        ...
        {/* DROPDOWN WEEK */}
        <div className="relative w-[110px] sm:w-[130px]" ref={weekRef}>
          <button
            type="button"
            onClick={() => {
            ...
            className="w-full bg-background border border-input rounded-xl px-2.5 py-2 text-[11px] font-bold text-primary flex items-center justify-between transition hover:border-primary cursor-pointer shadow-2xs"
          >
            <span className="truncate">
              {selectedWeekFilter === "ALL" ? "Semua Week" : `Week ${selectedWeekFilter}`}
            </span>
            <ChevronDown className={`h-3.5 w-3.5 text-primary transition-transform shrink-0 ${isWeekOpen ? "rotate-180" : ""}`} />
          </button>
        ...
        <button
          ...
          title="Reset Filter"
          className={`h-9 w-9 shrink-0 rounded-xl transition flex items-center justify-center cursor-pointer ${
            isFilterActive
