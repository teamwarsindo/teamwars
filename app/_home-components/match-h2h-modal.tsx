  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {/* BODY: DESKTOP 2-KOLOM LAPANG & SIMETRIS */}
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 sm:p-6 md:p-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
            ...
            {/* SISI KANAN (7 KOLOM DI DESKTOP): RIWAYAT MATCH SIMETRIS TOTAL */}
            <div className="lg:col-span-7 flex flex-col justify-between rounded-2xl border border-border bg-muted/20 p-4 sm:p-6 shadow-2xs h-full">
              <div className="space-y-3">
                ...
                {/* SUB-HEADER: IDENTITAS KEPEMILIKAN KOLOM DENGAN GRID PRESISI */}
                <div className="grid grid-cols-[1fr_70px_1fr] items-center px-4 py-2 bg-muted/40 rounded-xl border border-border/40 text-center">
                  <span className="text-xs sm:text-sm font-bold text-sky-600 dark:text-sky-400 truncate px-2 text-center">
                    {match.teamAName}
                  </span>
                  <span className="text-[10px] sm:text-[11px] font-black text-muted-foreground uppercase tracking-wider text-center">
                    PEKAN
                  </span>
                  <span className="text-xs sm:text-sm font-bold text-amber-600 dark:text-amber-400 truncate px-2 text-center">
                    {match.teamBName}
                  </span>
                </div>

                {/* LIST KARTU REPORT PER PEKAN (GRID 3 KOLOM TERKUNCI SIMETRIS) */}
                <div className="space-y-3 pt-1">
                  {displayWeeks.map((week) => (
                    <div
                      key={week}
                      className="grid grid-cols-[1fr_70px_1fr] items-center py-2.5 px-3 rounded-xl border border-border/50 bg-background/50 hover:bg-muted/60 transition shadow-2xs"
                    >
                      {/* SISI TIM A */}
                      ...
                      {/* KOLOM TENGAH: WEEK (LURUS PERSIS DENGAN HEADER PEKAN) */}
