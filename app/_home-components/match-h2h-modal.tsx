...
export function MatchH2HModal({
  ...
  return createPortal(
    ...
    >
      <div
        ref={modalContentRef}
        className="relative flex max-h-[92vh] w-full max-w-lg flex-col rounded-3xl border border-border bg-card text-card-foreground shadow-2xl overflow-hidden"
      >
        {/* HEADER MODAL */}
        <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-3 sm:px-5">
          <div className="min-w-0 pr-2">
            <span className="inline-flex items-center gap-1 rounded bg-primary/10 px-2 py-0.5 text-[9px] sm:text-[10px] font-bold text-primary">
              <Swords className="h-2.5 w-2.5 sm:h-3 sm:w-3" /> Week {match.weekNumber || currentWeek} • {match.groupName || "Group Stage"}
            </span>
            <p className="text-[10.5px] sm:text-xs text-muted-foreground font-medium mt-0.5">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3.5 text-xs">
          ...
          <div className="flex items-center justify-between rounded-2xl bg-muted/30 p-3 sm:p-4 border border-border">
            <div className="flex flex-col items-center flex-1 min-w-0 text-center gap-1 sm:gap-1.5">
              ...
              <span className="text-[11px] sm:text-xs font-medium truncate w-full" style={{ color: colorA }}>
                {match.teamAName}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center flex-1 min-w-0 text-center gap-1 sm:gap-1.5">
              ...
              <span className="text-[11px] sm:text-xs font-medium truncate w-full" style={{ color: colorB }}>
                {match.teamBName}
              </span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-3 space-y-1.5">
            ...
            <div className="flex items-center justify-between text-[10.5px] sm:text-xs font-medium">
              ...
              <span className="text-[9.5px] sm:text-[10px] text-muted-foreground font-normal">
          ...
          <div className="space-y-1.5 text-[10px] sm:text-[11px]">
            ...
            <div className="rounded-2xl border border-border bg-muted/20 divide-y divide-border overflow-hidden">
                ...
                <div className="flex items-center gap-0.5">
                  {statsA.form.length ? (
                    statsA.form.map((res, i) => (
                      <span
                        key={i}
                        className={`rounded px-1 py-0.2 text-[7.5px] sm:text-[8px] font-bold ${
                          res === "W"
              ...
              {/* 8. REPORT WEEK (FORMAT: [W/L] [LOGO] [NAMA TIM LAWAN] ↗) */}
              {pastWeeks.map((week) => {
                ...
                return (
                  <div key={week} className="flex items-center justify-between px-3 py-1.5 sm:py-2">
                    ...
                    <div className="flex-1 min-w-0 pr-1">
                      {itemA ? (
                        <a
                          ...
                          className={`inline-flex items-center gap-1 hover:opacity-80 transition text-[8.5px] sm:text-[9px] max-w-full ${
                            itemA.reportLink ? "cursor-pointer group" : "cursor-default"
                          }`}
                        >
                          <span
                            className={`rounded px-1 py-0.2 font-bold shrink-0 text-[7px] sm:text-[7.5px] ${
                              itemA.isWin
                                ? "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400"
                                : "bg-rose-500/20 text-rose-600 dark:text-rose-400"
                            }`}
                          ...
                          <span className="font-medium text-foreground truncate shrink max-w-[85px] sm:max-w-[130px] group-hover:underline">
                            {itemA.oppName}
                          </span>
                          {itemA.reportLink && (
                    ...
                    {/* LABEL TENGAH */}
                    <span className="text-muted-foreground font-normal text-[9px] sm:text-[9.5px] shrink-0 px-1">
                      Report Week {week}
                    </span>
                    ...
                    <div className="flex-1 min-w-0 pl-1 text-right">
                      {itemB ? (
                        <a
                          ...
                          rel="noopener noreferrer"
                          className={`inline-flex items-center justify-end gap-1 hover:opacity-80 transition text-[8.5px] sm:text-[9px] max-w-full ${
