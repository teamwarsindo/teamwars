...
export function calculateStandings(
  ...
  masterTeams.forEach((t) => {
    ...
    teamMap.set(tName, {
      ...
      matchWins: 0,
      matchLosses: 0,
      setWins: 0, // Bertindak sebagai Points Scored (Game Menang)
      setLosses: 0, // Bertindak sebagai Points Allowed (Game Kalah)
      roundDifference: 0, // Pts Diff
      points: 0, // Match Wins
    });
  });

  // 2. Akumulasi hasil match
  filteredSchedules.forEach((m) => {
    ...
    if (scoreA > scoreB) {
      itemA.matchWins += 1;
      itemA.points += 1; // Poin = 1 per Match Win
      itemB.matchLosses += 1;
    } else if (scoreB > scoreA) {
      itemB.matchWins += 1;
      itemB.points += 1; // Poin = 1 per Match Win
      itemA.matchLosses += 1;
    }
  });
