...
export async function handleGameCommand(interaction: any) {
  // 1. Validasi Akses Cepat (<1ms) persis seperti di /submit
  ...
  // 2. Background Task dibungkus waitUntil secara menyeluruh
  waitUntil(
    (async () => {
      try {
        ...
        if (subCommandName === 'add') {
          ...
          // Pengecekan SS Hand & Akumulasi Warning
          ...
          const isTeamAWon =
            reportData.teamA.score >= 10 || lineupB.every((p: any) => (p.remainingLife ?? 0) <= 0);
          const isTeamBWon =
            reportData.teamB.score >= 10 || lineupA.every((p: any) => (p.remainingLife ?? 0) <= 0);
          ...
          // Simpan ke KV dan sync tracker jika bukan simulasi sebelum kickoff
          if (!isBeforeKickoff) {
          ...
          // Jika simulasi admin sebelum kick-off, respon hanya di interaction follow-up (ephemeral)
          if (isBeforeKickoff && userIsAdmin) {
            if (appId && token) {
              await discordAPI(`/webhooks/${appId}/${token}/messages/@original`, 'PATCH', {
                embeds: [gameEmbed],
          ...
          // Kirim pesan publik ke channel pertandingan
          await discordAPI(`/channels/${channelId}/messages`, 'POST', { embeds: [gameEmbed] });

          // Update respon awal wasit
          if (appId && token) {
            await discordAPI(`/webhooks/${appId}/${token}/messages/@original`, 'PATCH', {
        ...
        console.error('Error in handleGameCommand background worker:', error);
        if (appId && token) {
          await discordAPI(`/webhooks/${appId}/${token}/messages/@original`, 'PATCH', {
  ...
  // 3. Langsung kirim respon Type 5 instan (<1ms) ke Discord
  return {
    type: 5,
    data: { flags: 64 },
  };
