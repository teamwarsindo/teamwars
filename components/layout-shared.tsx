...
// ==========================================
// 1. TOP BAR (STICKY & GLASSMORPHISM BLUR)
// ==========================================
  ...
  return (
    <header className="sticky top-0 z-50 flex w-full flex-col gap-2.5 border-b border-border/40 bg-background/85 px-4 py-3 backdrop-blur-md transition-all sm:px-6 lg:px-12">
      <div className="flex w-full items-center justify-between">
        <Link
          href="/"
          className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground hover:text-foreground transition-colors"
        >
      ...
      {/* Quick Nav Pills Bar */}
      <nav className="flex items-center justify-center gap-1.5 overflow-x-auto py-0.5 no-scrollbar sm:justify-center">
        {navLinks.map((link) => {
          const isActive =
            pathname === link.href ||
...
export function HeroHeader({ showDetails = true }: HeroHeaderProps) {
  return (
    <header className="mt-4 mb-6 flex flex-col items-center text-center sm:mt-6 lg:mb-8">
