...
export function PlayoffTab({ schedules = [], masterTeams = [] }: PlayoffTabProps) {
  // Hitung Standing Akumulatif menggunakan calculator terpusat
    ...
    <div className="flex flex-col gap-6 rounded-3xl border border-border bg-card p-4 sm:p-6 overflow-x-auto shadow-xl">
      <div className="border-b border-border pb-3 text-center sm:text-left">
        ...
        <p className="text-[11px] text-muted-foreground mt-1 font-semibold">
          Diisi otomatis dari Top 2 Tiap Divisi (Kualifikasi Langsung QF) &amp; Top 8 Wildcard Global (Round 1).
        </p>
      </div>

      {/* STRUKTUR BRACKET BAGAN DENGAN GARIS PENGHUBUNG (CONNECTOR LINES) */}
      <div className="min-w-[900px] grid grid-cols-4 gap-6 text-xs py-4 px-2 relative">
        {/* ================= ROUND 1: PLAY-INS ================= */}
