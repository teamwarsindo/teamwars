...
export function TournamentView({
  ...
  useEffect(() => { fetchTournamentData(); }, []);

  const handleForceResetSchedules = async () => {
    const res = await Swal.fire({
      title: "REGENERATE JADWAL DEFAULT?",
      text: "Sistem akan membuat ulang jadwal Group A & B (Rabu-Sabtu jam 20:00 WIB).",
      icon: "warning",
  ...
  if (isLoading) return <div className="p-8 text-center text-xs font-bold text-primary animate-pulse">⏳ Memuat Data Turnamen...</div>;
