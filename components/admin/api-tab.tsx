...
export function ApiTab() {
  ...
  // State untuk Pop-up Modal Konfirmasi
  ...
  // Fungsi dipanggil saat tombol "Run API" diklik -> Munculkan Modal
  const triggerRunApi = () => {
    if (!selectedRoute) return
    setShowConfirmModal(true)
  }

  // Fungsi sesungguhnya untuk menembak API (Dipanggil dari dalam Modal)
  ...
  return (
    <>
          ...
          <div className="lg:col-span-2 rounded-xl border border-primary/20 bg-black/80 flex flex-col shadow-[inset_0_0_30px_-10px_rgba(0,0,0,1)] h-[40vh] lg:h-[60vh] overflow-hidden">
            ...
            <div className="flex items-center justify-between bg-zinc-900 px-3 py-2 border-b border-zinc-800 shrink-0">
              ...
              <div className="flex items-center gap-2">
                {selectedRoute && (
                  <button
                    onClick={triggerRunApi} // Panggil Modal, bukan jalankan langsung
                    disabled={isTesting}
      ...
      {/* POP-UP MODAL KONFIRMASI (Custom UI) */}
      {showConfirmModal && (
          ...
          <div className="bg-background border border-primary/30 w-full max-w-sm rounded-2xl shadow-[0_0_40px_-10px_rgba(220,38,38,0.3)] overflow-hidden">
            <div className="p-5 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Kamu akan menjalankan request ke endpoint:
              </p>
            ...
            <div className="grid grid-cols-2 border-t border-border/50 divide-x divide-border/50">
              <button 
                ...
                className="p-3 text-sm font-semibold hover:bg-muted/50 transition-colors"
              >
                Batal
              </button>
              <button 
                onClick={executeApi}
                className="p-3 text-sm font-semibold text-primary hover:bg-primary hover:text-white transition-colors"
              >
                Eksekusi
              </button>
            </div>
